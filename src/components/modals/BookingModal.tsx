  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(scriptURL, {
        method: "POST",
        // Notice we are omitting {"Content-Type": "application/json"} to bypass Google's CORS preflight limitation.
        body: JSON.stringify({
          name,
          email,
          phone,
      ...
      <div className="bg-white p-6 md:p-10 w-full max-w-5xl shadow-2xl relative max-h-[90vh] overflow-y-auto animate-fadeSlide" data-lenis-prevent>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mt-4">
          ...
          >
            ...
            {openDate && (
              <div
                className="absolute top-0 left-0 w-full bg-[#F9F5EC] z-50 shadow-lg p-6 flex justify-center"
                onClick={(e) => e.stopPropagation()}
              >
