...
const MomentsSection = () => {
    const moments = [
        {
            id: 1,
            image: "https://images.unsplash.com/photo-1571896349842-6e53ce41e887?q=80&w=2071&auto=format&fit=crop",
            ...
            renderItem={(moment, isActive) => (
                <div
                    className={`relative w-full aspect-[3/4] overflow-hidden transition-all duration-500 ease-out 
                        ${isActive ? "scale-100 z-10 shadow-2xl" : "scale-90 opacity-60 grayscale-[30%] z-0"}
                    `}
                >
                    <img 
                        src={moment.image} 
                    ...
                    */}
                    <div className={`absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                        <div className="absolute bottom-8 left-8 text-white">
                            <h3 className="text-2xl font-serif tracking-wide">{moment.title}</h3>
                        </div>
                    </div>
